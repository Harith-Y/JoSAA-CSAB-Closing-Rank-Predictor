"""
CSAB 2026 Current-Year Scraper
URL: https://admissions.nic.in/csabspl/Applicant/seatallotmentresult/currentorcr.aspx

Appends rows to data/csab_ranks.csv with Year=2026 and Special Round columns.
Resume-safe: skips rounds already present in the CSV.

Usage:
    python scripts/scrape_csab_2026.py
"""
...
OUTPUT_FILE = os.path.join(DATA_DIR, "csab_ranks.csv")
YEAR_LABEL  = "2026"
...
def main():
    ...
    if completed:
        print(f"Resuming - 2026 CSAB rounds already done: {sorted(completed)}")
    ...
    with sync_playwright() as p:
        ...
        print(f"Special Rounds for 2026: {[r[1] for r in rounds]}")
        ...
        print(f"\nDone!  CSAB 2026 data appended to: {OUTPUT_FILE}")
        browser.close()
